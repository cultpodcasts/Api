...
.DESCRIPTION
  Calls the deployed Api Worker — ephemeral isolates, no probe Worker deploy.
...
.PARAMETER IncludeUsFetch
  Include cfUsFetch leg (requires SCRAPE_US on Api + expectedPop.cfUsFetch).
...
.PARAMETER ExpectedUsLocs / ExpectedUsColos
  Allowlists for cfUsFetch preflight.
#>
param(
